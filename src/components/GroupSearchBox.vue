...
<script lang="ts">
  ...
  @Component({
    ...
    methods: mapActions('searchGroup', ['setKeyword']),
    computed: mapGetters('searchGroup', ['getKeyword']),
  })
  export default class GroupSearchBox extends Vue {
    ...
    private changePlaceholder(): void {
      if (this.disabled) {
        this.placeholder = 'Please register a new Group.'
      } else {
